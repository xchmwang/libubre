...
using nr_ret_type = std::tuple<int32_t, std::string, std::vector<std::shared_ptr<nr_info_t>>>;
...
nr_ret_type entry_point_nr_impl(compatible_uint64_t start_block,
  ...
  auto bc_path = neb::fs::join_path(nbre_path, "dummy_db/dummy_default_random_.db");
